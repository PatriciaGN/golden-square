...
RSpec.describe TimeError do
    describe "error" do
        it "returns the difference in seconds between the remote time and time on the computer" do
            ...
            time = Time.new(2022, 9, 22, 13, 46, 33.498637)
            time_error = TimeError.new(fake_requester)
            expect(time_error.error(time)).to eq -2.217348082922399e-15
        end
        ...
        it "10 seconds difference" do
            ...
            fake_requester.get(URI("https://worldtimeapi.org/api/ip"))
            time = Time.new(2022, 9, 22, 13, 46, 33.498637)
            time += Rational(10, 86400)
            time_error = TimeError.new(fake_requester)
            expect(time_error.error(time)).to eq -0.00011574074074295809
        end
    end
end
